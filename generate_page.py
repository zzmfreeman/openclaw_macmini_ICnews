...
<body>
  ...
  <script>
    ...
    function render(){{const d=state.filtered||state.data;if(!d)return;if(!d.items)d.items=[];
      ...
      const gl=d.glossary_list||[];
      if(gl.length){{el('glossarySection').innerHTML=`<div style="font-size:14px;font-weight:700;margin-bottom:12px">📚 行业术语表</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">${{gl.map(g=>`<div style="font-size:13px;padding:8px;border:1px solid var(--border);border-radius:6px"><span style="font-weight:600;color:var(--text)">${{esc(g.term)}}</span> ${{esc(g.desc)}}</div>`).join('')}}</div>`;}}
      ...
      const ri=(it,i)=>{const gl=Array.isArray(it.glossary)?it.glossary:typeof it.glossary==="string"?[it.glossary]:[];return `<div class="item" id="item-${{i}}"><div class="header"><div class="title2">${{esc(it.title)}}</div><div class="meta-row"><span>#${{i+1}}</span><span>📰 ${{esc(it.source||'')}}</span><span>📅 ${{esc(it.published||'')}}</span>${{it.url?`<a href="${{esc(it.url)}}" target="_blank">🔗 原文</a>`:''}}${{(it.tags||[]).map(t=>`<span class="badge badge-tag">${{esc(t)}}</span>`).join('')}}</div></div><div class="summary">${{it.summary}}</div><div class="box box-insight"><div class="lbl">💡 洞见</div><ul>${{(it.insights||[]).map(x=>`<li>${{esc(x)}}</li>`).join('')}}</ul></div><div class="box box-action"><div class="lbl">🎯 建议</div><ul>${{(it.actions||[]).map(x=>`<li>${{esc(x)}}</li>`).join('')}}</ul></div></div>${{(it.glossary||[]).length?`<div class="glossary-box"><div class="glossary-title">📚 术语</div>${{gl.map(g=>typeof g==="string"?`<div class="glossary-item"><span class="glossary-term">${{esc(g)}}</span></div>`:`<div class="glossary-item"><span class="glossary-term">${{esc(g.term)}}</span>${{esc(g.desc)}}</div>`).join('')}}</div>`:''}}`};
